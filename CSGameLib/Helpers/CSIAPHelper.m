...
- (id)initWithProductIdentifiers:(NSSet *)productIdentifiers {
    if ((self = [super init])) {
        ...
        for (NSString * productIdentifier in _productIdentifiers) {
            ...
            NSString* password = [CSSFHFKeychainUtils getPasswordForUsername:productIdentifier andServiceName:@"CSIAPHelper" error:nil];
...
-(BOOL)isPurchasedProductsIdentifier:(NSString*)productID
{
    ...
    NSString* password = [CSSFHFKeychainUtils getPasswordForUsername:productID andServiceName:@"CSIAPHelper" error:nil];
    if([password isEqualToString:@"YES"])
    {
        productPurchased = YES;
    }
...
- (void)provideContentWithTransaction:(SKPaymentTransaction *)transaction {
    ...
    [CSSFHFKeychainUtils storeUsername:productIdentifier andPassword:@"YES" forServiceName:@"CSIAPHelper" updateExisting:YES error:nil];
...
- (void)clearSavedPurchasedProductByID:(NSString*)productIdentifier {
    ...
    [CSSFHFKeychainUtils deleteItemForUsername:productIdentifier andServiceName:@"CSIAPHelper" error:nil];
...
- (void)checkReceipt:(NSData*)receiptData AndSharedSecret:(NSString*)secretKey onCompletion:(checkReceiptCompleteResponseBlock)completion
{
    ...
    NSString* sjsonStr = [[NSString alloc] initWithData:receiptData encoding:NSUTF8StringEncoding];
    NSLog(@"%@",sjsonStr);
    NSError *jsonError = nil;
    NSString *receiptBase64 = [CSGTMBase64 stringByEncodingData:receiptData];
